...
Reaction rules define the behavior of a bigraphical reactive system specification and are defined over the same signature as the system model (i.e., the initial bigraph state or configuration).
...
- [`org.bigraphs.framework.core.reactivesystem.AbstractReactionRule`](https://bigraphs.org/software/bigraph-framework/apidocs/org/bigraphs/framework/core/reactivesystem/AbstractReactionRule.html)
...
Additionally, the post-conditions, i.e., the result is easily specified using the right-hand side of the rule (i.e., the reactum).

Another variant provide so-called Conditional Rules ([`org.bigraphs.framework.core.reactivesystem.ConditionalParametricRuleDecorator`](https://bigraphs.org/software/bigraph-framework/apidocs/org/bigraphs/framework/core/reactivesystem/ConditionalParametricRuleDecorator.html)).
